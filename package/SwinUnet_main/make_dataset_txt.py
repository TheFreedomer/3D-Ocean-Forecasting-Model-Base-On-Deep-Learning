...
def npz_csv():
    datasets_config = {
        ...
        #     'num_classes': 3 + all_varaible_stack_lead1,  # plus background
        #     'predict_head': all_varaible_stack_lead1
        # },
        'MRI_MM': {
...
def process_file(config, split_path, filepath, file_samples):
    ...
    if split_path and filename not in file_samples:
        ...
        for z_index in range(img.shape[1]):
            ...
            #                 f"{DirUtils.split_extension(split(filepath)[-all_varaible_stack_lead1], suffix=f'_{z_index:04}_seg')}")
